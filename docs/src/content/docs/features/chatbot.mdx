...
| Variable | Value |
|---|---|
| `{{phone_number}}` | The contact's WhatsApp number in E.164 format (e.g. `+919876543210`). |
| `{{_flow_id}}` | UUID of the running flow. |
| `{{_flow_name}}` | Human-readable name of the flow. |

Anything else has to come from somewhere — typically:
- An earlier flow step with a `Store As` configured (user input gets saved
  under that key for later steps).
- A previous API step's `response_mapping` (mapped fields are persisted
...
<Aside type="note">
  Substitution is verbatim — there's no URL-encoding pass. For
  `{{phone_number}}` (which always starts with `+`), this is fine inside
  URL paths but not inside query strings, where some servers parse `+`
  as a space. Prefer `https://api.example.com/users/{{phone_number}}`
  over `?phone={{phone_number}}` when possible.
</Aside>
